...
└── _harness/                           # 프로젝트 산출물
    ...
    ├── permissions.md                  # 단계별 권한 + 보안 정책 결정
    ├── status.md                       # /se:backup 스냅샷 (4단계)
    ├── integrity_alert.md              # 매니페스트 불일치 알림 (2단계)
    ├── projects/                       # 프로젝트별 .env (Git 제외)
    │   └── {프로젝트명}/.env
    ├── continuous_learning/            # (9단계)
    │   ├── whitelist.yaml
    │   ├── decisions_index.md
    ...
    ├── security_engineer/
    │   ├── 01_plan.md ~ 05_document.md  # 페르소나 단계별 에이전트 정의
    │   ├── execute_library.md          # 외부 라이브러리·MCP 라이브러리
...
|---|---|---|---|---|
| `/se:plan` | Opus 4.7 | `01_plan.md`, (이전) `plan.md` | 없음 | (대화에만 — 확정 전 미저장) |
| `/se:critique` | Opus 4.7 | `02_critique.md`, `plan.md`(슬림) + 대상 `plan/{N}.md` 1개 | `--diff` 모드: `git diff` 직전 라운드만 | `history.md` (비판 이력 추가) |
| `/se:confirm` | Sonnet 4.6 | `commands/se/confirm.md` 절차 | 없음 | `plan.md`, `plan/{N}.md`, `plan_risks.md`, `history.md` |
| `/se:execute N` | Sonnet 4.6 | `03_execute.md`, `plan/{N}.md` 1개 | `execute_library.md`(외부 의존성 시), `permissions.md`(권한 변경 시), `rules/common/security.md`(7단계 후) | 단계 산출물, `sbom_report.md`, `history.md`, `plan.md` 로드맵 체크 |
| `/se:review` | Sonnet 4.6 | `04_review.md`, 단계 산출물, `plan_risks.md` | `sbom_report.md`, `archive/`(필요 시) | `review_report.md` (압축 요약), `archive/{YYYYMM}/` (원본 이동) |
| `/se:doc [A/B/C/D]` | Haiku 4.5 | `05_document.md`, `review_report.md`, `plan.md`(슬림), `plan_risks.md` | 없음 | `_harness/security_engineer/docs/` |
| `/se:backup` | Sonnet 4.6 | `commands/se/backup.md` 절차 | (변경 없으면 즉시 skip) | `status.md` (변경 시에만), `history.md` 통합 |
| `/se:summary` | Sonnet 4.6 | `commands/se/summary.md` + 트랜스크립트 | 없음 | `_harness/security_engineer/docs/summary_*.md` |
| `/se:note <type>` | Sonnet 4.6 | `commands/se/note.md` + 직전 10분 메타데이터 | `quarantine/` (외부 출처 발견 시) | `decisions_index.md` 또는 `quarantine/` |
| `/se:learn sonnet\|opus` | 인자 따라 | `commands/se/learn.md`, `quarantine/` | 없음 | `quarantine/` (수락/거부 표시) |
| `/se:learn audit` | Sonnet 4.6 | `commands/se/learn-audit.md`, `cautions.md`, `decisions_index.md` | 없음 | `cautions.md` (점검 결과 추가) |
| `/se:learn cleanup` | Sonnet 4.6 | `commands/se/learn-cleanup.md` | `quarantine/`, `cautions.md` | `archive/{YYYYMM}/` 이동 |
| `/se:learn rollback <hash>` | Sonnet 4.6 | `commands/se/learn-rollback.md` + git 로그 | 없음 | git revert 커밋 |
| `/teacher:ask` | Sonnet 4.6 | `01_ask.md` | 없음 | (대화만) |
| `/teacher:doc` | Sonnet 4.6 | `02_document.md` | 없음 | `_harness/student/docs/` |
...
| 우선순위 | 파일 | 언제 |
|---|---|---|
| ★★★ | [CLAUDE.md](CLAUDE.md) | 항상 (모든 세션 자동 로드) |
| ★★★ | [_harness/security_engineer/plan.md](_harness/security_engineer/plan.md) | 모든 단계 — 헤더+로드맵 확인 |
| ★★★ | [_harness/security_engineer/plan/{N}.md](_harness/security_engineer/plan/) | 진입할 단계 본문 |
| ★★ | [_harness/history.md](_harness/history.md) | 세션 재개 시 가장 먼저 |
| ★★ | [_harness/permissions.md](_harness/permissions.md) | 권한·보안 정책 변경 시 |
| ★★ | [_harness/security_engineer/plan_risks.md](_harness/security_engineer/plan_risks.md) | review/doc 단계 또는 의사결정 시 |
| ★ | [_harness/security_engineer/execute_library.md](_harness/security_engineer/execute_library.md) | 외부 라이브러리·MCP 사용 시 |
| ★ | [_harness/security_engineer/review_report.md](_harness/security_engineer/review_report.md) | 검토 직후 |
| ★ | [.env.example](.env.example) | 새 시크릿 슬롯 추가 시 |
...
| [_harness/security_engineer/plan_risks.md](_harness/security_engineer/plan_risks.md) | **단계 종료 시** | 식별 리스크 vs 감수 리스크 재평가 | 리스크 가시성 |
...
| [_harness/integrity_alert.md](_harness/integrity_alert.md) | **세션 시작 시 자동 확인** | 매니페스트 불일치 발생 여부 | 변조 감지 즉시 대응 |
...
| `.env` 위치 | `_harness/projects/{프로젝트명}/.env` | 프로젝트 격리 + Git 강제 제외 | 변경 없음 |
...
2. 새 프로젝트 시크릿: `cp .env.example _harness/projects/{프로젝트명}/.env` → 값 채움 (Git 제외 강제)
3. 워크플로 시작: `/se:plan <목표>` → `/se:critique` → `/se:confirm` → `/se:execute 1`
4. 진행 상태 확인: `_harness/security_engineer/plan.md` 로드맵 표
5. 세션 재개: `_harness/history.md` 가장 위 항목부터 읽기

---

## 12. 도움말 / 변경 이력

- 진행 이력: [_harness/history.md](_harness/history.md)
- 현 계획: [_harness/security_engineer/plan.md](_harness/security_engineer/plan.md) (v5)
- 리스크 전체: [_harness/security_engineer/plan_risks.md](_harness/security_engineer/plan_risks.md)
- 권한·정책: [_harness/permissions.md](_harness/permissions.md)
